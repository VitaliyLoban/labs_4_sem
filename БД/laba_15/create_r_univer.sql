...
ORDER BY  Case [Коэфициенты от 4 до 6]
 ...
 from Предприятия
  select * from Предприятия_
 ----
insert  Предприятия_ values('Басалай','Алексей')
select * from Предприятия_
...
declare name_subject cursor for select SUBJECT.SUBJECT from SUBJECT  where SUBJECT.PULPIT = 'ИСиТ'
open name_subject 
fetch name_subject into @subject 
print 'Название предметов'
...
for select SUBJECT.SUBJECT, SUBJECT.SUBJECT_NAME,SUBJECT.PULPIT from SUBJECT where SUBJECT.PULPIT = 'ИСиТ'
open subject
print 'Количество строк:'+cast(@@CURSOR_ROWS as varchar(5))
----
update SUBJECT set SUBJECT.SUBJECT ='Database' where SUBJECT.PULPIT='ПОиСОИ'
...
declare  @tc int, @rn char(50);  
	     ...
	     SUBJECT.SUBJECT_NAME from dbo.SUBJECT where SUBJECT.PULPIT = 'ИСиТ' 
	open navigation;
...
declare @tn int, @tc date, @tk nvarchar(max);   
	...
	begin try
	 begin tran               
	   delete SUBJECT where SUBJECT = 'МП'; 
	   insert SUBJECT values ('МП','Математическое программирование',  'ИСиТ');
	   commit tran;              
	end try
	begin catch
	    print 'ошибка: '+ case 
...
begin try
  begin tran                                                           
	delete SUBJECT where SUBJECT ='ПЗ'; 
	set @point = 'p1'; save tran @point;          
	insert SUBJECT values ('ПЗ',' Представление знаний в компьютерных системах',  'ИСиТ');
 ...
 -- A ---
	...
	-------------------------- t1 ------------------
	  select @@SPID, 'insert Subject' 'результат', * from SUBJECT 
	                                                             where SUBJECT='МП';
	  select @@SPID, 'update Subject'  'результат', 
...
--- B --	
	begin transaction 
	select @@SPID
	insert SUBJECT values ('ПС', 'Психология','ИСиТ'); 
	update SUBJECT set SUBJECT = 'ПИСС' 
                           where SUBJECT = 'ПИС' 
	rollback;
	--5
...
-- A ---
	...
	from SUBJECT  where SUBJECT = 'МП';
	commit; 
	--- B ---	
	begin transaction 	  
	-------------------------- t1 --------------------
          update SUBJECT set SUBJECT = 'МПП' 
                                       where SUBJECT = 'ПЗ' 
          commit; 
	-------------------------- t2 --------------------	
	...
	begin transaction 
	select SUBJECT.SUBJECT_NAME from SUBJECT where SUBJECT = 'МП';
	-------------------------- t1 ------------------ 
	-------------------------- t2 -----------------
	select  case
          when SUBJECT.SUBJECT = 'ИНФ' then 'insert  Subject'  else ' ' 
end 'результат', SUBJECT.SUBJECT_NAME from SUBJECT  where SUBJECT_NAME = 'Информационные технологии';
	commit; 
	--- B ---	
	begin transaction 	  
	-------------------------- t1 --------------------
          insert SUBJECT values ( 'C#', 'C# programming', 'ИСиТ');
          commit; 
	-------------------------- t2 --------------------
...
use master;
	...
	begin transaction 
	select *  from SUBJECT where SUBJECT = 'МП';
	-------------------------- t1 ------------------ 
delete SUBJECT where SUBJECT = 'МПП';  
          insert SUBJECT values ('МПП', 'Представление знаний в компьютерных системах', 'ИСиТ');
          update SUBJECT set SUBJECT_NAME = 'Представление знаний в системах' where SUBJECT = 'МПП';
	-------------------------- t2 -----------------
	select  SUBJECT from SUBJECT  where SUBJECT.SUBJECT = 'МПП';
	commit; 
	--- B ---	
	begin transaction 	  
	-------------------------- t1 --------------------
...
----------------//////////////////////////----------------
	...
	begin transaction 
		delete SUBJECT where SUBJECT = 'МПП';  
         insert SUBJECT values ('МПП', 'Представление знаний в компьютерных системах', 'ИСиТ');
        update SUBJECT set SUBJECT_NAME = 'Представление знаний в системах' where SUBJECT = 'МПП';
          	select SUBJECT  from SUBJECT where SUBJECT = 'МП';
	-------------------------- t1 -----------------
		select SUBJECT  from SUBJECT where SUBJECT = 'МП';
	-------------------------- t2 ------------------ 
	commit; 	
...
----------///---------------------------
begin tran                                          
 insert SUBJECT values ('ПЗКС', 'Представление знаний в компьютерных системах', 'ИСиТ')
begin tran                                          
update SUBJECT set SUBJECT = 'ЗКС' where SUBJECT = 'ПЗКС';
commit;                                            
if @@trancount > 0  rollback;        
select   (select count(*) from SUBJECT where SUBJECT = 'ЗКС') 'Предмет 1', 
(select count(*) from SUBJECT  where SUBJECT = 'МП') 'Предмет 2'; 

----14

CREATE PROCEDURE PPRED
as  begin 
declare @k int = (select count (*) from Предприятия);
select * from Предприятия;
return @k;
end;
declare @k int = 0;
    EXEC @k=PPRED;   
    print 'кол-во predpriyatii = '+cast(@k as varchar(3));


	CREATE PROCEDURE P_INSERT
 @a nvarchar(20), @n int, @c nvarchar(20),@t nvarchar(1)    as  declare @rc int=1;                            
begin try                                     
insert into Предприятия (Предприятия.Название_предприятия,Предприятия.Телефон,Предприятия.Контактное_лицо,Предприятия.Пол)
values (@a,@n,@c,@t)
return @rc;
end try  
begin catch              -- обработка ошибки 
  ...
  print 'имя процедуры : ' + error_procedure();
   ...
   declare @rc int;  
exec @rc=P_INSERT @a='найк',@n=12312,@c='andrey',@t='м'
...
begin catch              -- обработка ошибки 
  ...
  print 'имя процедуры : ' + error_procedure();
   ...
   declare @rc int;  
exec @rc=PAUDITORIUM_INSERT @a='100-3a',@n='100-3a',@c=100,@t='ЛК'
print 'код ошибки : ' + cast(@rc as varchar(3)); 
...
declare @rc int; 
exec @rc=PAUDITORIUM_INSERTX @a='101-2', @n ='101-2', @1c = 24 ,@t='=ЛК',@tn='Лекционная'; 
print 'код ошибки=' + cast(@rc as varchar(3));
...
as
 ...
 go  
     select  PULPIT.PULPIT, dbo.FCTEACHER(PULPIT) [Количество преподавателей]    from PULPIT       
...
alter function FACULTY_REPORT(@c int) returns @fr table
	...
	as begin 
		declare cc cursor static for select FACULTY from FACULTY where dbo.COUNT_STUDENTS(FACULTY) > @c; 
		declare @f varchar(30);
	    open cc;  -- идем по факультетам
		    fetch cc into @f; -- записали название факультета в @f
	        while @@fetch_status = 0
	        begin
	            insert @fr values -- вставляем данные в таблицу
				(@f,  -- факультет
				dbo.COUNT_PULPIT(@f), -- количество кафедр
	            dbo.COUNT_GROUP(@f), -- количество груп
				dbo.COUNT_STUDENTS(@f), -- количество студентов 
	            dbo.COUNT_PROF(@f)); -- количество специальностей
	            fetch cc into @f;  -- идем на следующий факультет
	         end;   
		close cc;
		return; 
	end;
...
as
begin
declare @rc int = (select  count (*) from Показатели
		where Коэфициент_показателя = ISNULL(@count, Коэфициент_показателя));
		return @rc;
		end;
		select  dbo.sumNum (Null)
		select *, dbo.sumNum(Коэфициент_показателя) from Показатели

		create function FFA(@f varchar(50), @p varchar(50)) returns table 
      as return 
          select Банковские_реквизиты.Предприятие, Предприятия.Название_предприятия 
          from Банковские_реквизиты left outer join Предприятия 
          on Банковские_реквизиты.Предприятие = Предприятия.Название_предприятия
          where Банковские_реквизиты.Предприятие = isnull(@f,Банковские_реквизиты.Предприятие) 
          and  Предприятия.Название_предприятия = isnull(@p,  Предприятия.Название_предприятия);
   select * from dbo.FFA(NULL,NULL);
   select * from dbo.FFA(NULL,'Богданович_ИП');
   select * from dbo.FFA ('Макаров_ИП',NULL)

    create function COUNT(@f varchar(25)) returns int
    as begin  declare @rc int = 0;
    set @rc = (select count(Банковские_реквизиты.Предприятие) 
	        from Банковские_реквизиты  
			join  Предприятия on Банковские_реквизиты.Предприятие=Предприятия.Название_предприятия
            where Предприятие  = @f) ;  
	    return @rc         
	    end
	    declare @f int=dbo.COUNT('Лобан_ИП');
		select Банковские_реквизиты.Предприятие, 
        dbo.COUNT(Банковские_реквизиты.Предприятие)from Банковские_реквизиты;	